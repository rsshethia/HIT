...
import { messageTypes, allSegments } from "@/data/hl7-data";
...
const HL7SegmentMapper = () => {
  ...
  useEffect(() => {
    ...
    // Create nodes for message types
  ...
  useEffect(() => {
    ...
    node.append("circle")
      .attr("r", (d: any) => d.type === 'segment' ? 8 + (d.value * 0.5) : 12)
      .attr("fill", (d: any) => d.type === 'segment' ? "#4299e1" : "#f6ad55")
      .attr("stroke", "#fff")
      .attr("stroke-width", 1.5);

    // Add text labels to nodes
    node.append("text")
      .attr("dy", (d: any) => d.type === 'segment' ? -12 : 16)
      .attr("text-anchor", "middle")
      .attr("font-size", (d: any) => d.type === 'segment' ? "10px" : "12px")
      .attr("font-weight", (d: any) => d.type === 'segment' ? "normal" : "bold")
      .text((d: any) => d.id);
    ...
    node.append("title")
      .text((d: any) => {
        if (d.type === 'segment') {
          return `${d.name}\nUsed in ${d.value} message types`;
        } else {
          return `${d.name}\nContains ${d.segments ? d.segments.length : 0} segments`;
        }
      });
